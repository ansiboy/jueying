...
{
    public static string Format(string originFileName, string pathFormat, int imgWidth = 0, int imgHeight = 0)
    {
        ...
        if (imgWidth > 0)
            pathFormat = pathFormat.Replace("{width}", imgWidth.ToString());

        if (imgHeight > 0)
            pathFormat = pathFormat.Replace("{height}", imgHeight.ToString());
